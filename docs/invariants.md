...
| INV-H4 | `GET /me/billing` maps per the FIX-P1 table: `active → "active"`, `past_due / grace_period → "past_due"`, `cancelled → "canceled"`, `failed → "failed"`, else → `"none"`. | BD1, users_store, billing_config | `test_runtime_inv_http.py::TestINV_H4_MeBillingFailedMapping` (parametrized × 5); `test_fix_p1_billing_surface_hardening.py::TestMeBillingStatusMapping` |
