...
//
// Rules layer: where immunity (Card 60) asks "can the system prevent
// future instability?", governance asks "is the system holding the
// invariants that define structural safety?". The distinguishing
// section over immunity is:
//
//   [Structural Invariants]   fixed rules that define safe operation
//
// Per-dimension vocabulary shifts from immunity-words ("strong/weak X
// immunity") to compliance/adherence-words ("full invariant
// compliance", "strong threshold adherence", etc.). The trajectory
// tail uses "(projected)" instead of "(improving)/(deteriorating)";
// direction lives in the summary sentence.
//
// Eleven sections:
//
//   [Governance Level]            LOW / LOW-MEDIUM / MEDIUM / MEDIUM-HIGH / HIGH
//   [Structural Invariants]       fixed rule lines
//   [Governance Profile]          per-dimension compliance words
//   [Governance Trajectory]       current → next → two-step
//   [Governance Drivers]          maintenance actions in effect
//   [Governance Inhibitors]       active vulnerabilities
//   [Governance Thresholds]       numeric safe bounds
//   [Governance Breach Conditions] failure modes the system has shown
//   [Governance Reinforcement]    maintain-actions for active dims
//   [Governance Decay]            forward-looking weakening risks
...
function collectDrivers(t: Trends): string[] {
  const out: string[] = [];
  // System-level driver: at least one dimension is being actively
  // controlled. Reads as the broader "immunity layer is doing its job".
...
// Per Card 61 spec: trajectory tail is "(projected)" when the level
// changes across the three steps, "(stable)" when it doesn't. The
// direction word lives in the summary sentence, not the trajectory.
...
// ----- Profile (per-dimension compliance vocabulary) --------------------
...
interface GovernanceProfile {
  crit:   Strength;  // → invariant compliance
  drift:  Strength;  // → threshold adherence
  upper:  Strength;  // → upper-branch containment
  vol:    Strength;  // → volatility control
}
...
// CZ invariant has its own vocabulary: full / partial / weak.
...
// ----- Breach Conditions + Reinforcement + Decay -------------------------

function collectBreachConditions(t: Trends): string[] {
  const out: string[] = [];
  // Threshold violation: any dimension has ever crossed its threshold.
...
function collectDecay(t: Trends): string[] {
  const out: string[] = [];
  // Card 61 frames decay around invariant weakening rather than
  // dimension-specific re-emergence (the Card 60 framing). The
  // threshold-weakening line fires whenever any threshold has been
  // pressured historically.
...
export function buildStructuralGovernance(
  ...
  // `diffs` + `immunity` kept on the signature for spec parity;
  // Phase-1 governance derives from the structured signature trends
  // directly.
  void diffs;
  void immunity;
  ...
  // Never-active short-circuit: baseline governance is HIGH because
  // every invariant has been respected by default.
