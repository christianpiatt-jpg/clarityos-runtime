...
//
// First reasoning-layer instrument: compares two Card 50 per-run
// signatures and produces a hierarchical text-only diff covering
// fingerprint, hydraulic, pressure band, four count signatures, and a
// rule-based identity-shift classification.
//
// Output layout (matches the spec example):
//
//   === Structural Signature Diff ===
//   Run X → Run Y
//
//   [Fingerprint Diff]                <from.fp>  →  <to.fp>
//   [Hydraulic Diff]                  <from.regime> → <to.regime>
//   [Pressure Band Diff]              <from.band>  →  <to.band>
//   [Critical-Zone Diff]              N → M  (increase / decrease / no change)
//   [Upper-Branch Diff]               …
//   [Volatility Diff]                 …
//   [Drift Diff]                      N → M  (onset / increase / decrease / no change)
//   [Phase-Transition Diff]           <fromLabel> → <toLabel>
//
//   [Identity Shift Classification]
//   Type: <Structural Escalation | Relaxation | Drift | Stabilization | No significant shift>
//   Notes:
//   - <reasoning line>
//   - <reasoning line>
...
// Generic count comparator used for crit / upper-branch / volatility.
...
// Drift gets specialised wording — "(onset)" when going from 0 to
// positive matches the spec example and operator vocabulary.
...
function classify(
  ...
  // Priority order: drift > escalation > relaxation > stabilization >
  // no shift. A single transition may match several rules but we pick
  // the dominant label for `Type:`; all matches show up in `Notes:`.
  let type: string;
...
// Card 52 — structured per-pair diff exposed for the signature
// overlay engine. Carries enough state to re-render the diff text
// AND derive the brief overlay descriptions Card 52 uses.
