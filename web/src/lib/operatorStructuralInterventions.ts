...
//
// Action layer: turns the Card 56 causal chains into recommended
// operator-grade interventions. Each section maps a dimension /
// run / identity pattern to a fixed-vocabulary action so the
// recommendations stay stable across runs and surfaces.
//
// Five sections:
//
//   [Primitive-Level Interventions]    Phase-1 sentinel — primitive IDs
//                                      require lineage + overlay context.
//   [Run-Level Interventions]          per non-LOW run: action bullets.
//   [Structural-Dimension Interventions] per-dimension recommendations.
//   [Identity-Shift Interventions]     identity-pattern actions.
//   [System-Level Intervention Summary] ranked top-priority actions.
...
// ----- Structural-Dimension Interventions --------------------------------
...
// ----- System-Level Intervention Summary ---------------------------------
...
function buildSummary(
  ...
  // Surface explicit Card 51 volatility-spike notes as evidence even
  // if the count chain didn't strictly rise across the whole window.
...
export function buildStructuralInterventions(
  ...
  // `causality` kept on the signature for spec parity; interventions
  // derive from the structured signature + diff sequence directly.
  void causality;
