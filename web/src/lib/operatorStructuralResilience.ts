...
//
// Durability layer: scores the system's projected ability to resist
// future instability. Drivers (recovery signals) accumulate while
// inhibitors (residual instabilities) detract.
//
// Nine sections:
//
//   [Resilience Score]               LOW / MEDIUM / MEDIUM-HIGH / HIGH
//   [Resilience Profile]             per-dimension resistance notes
//   [Resilience Trajectory]          current → projected
//   [Resilience Drivers]             stabilization signals
//   [Resilience Inhibitors]          residual instabilities
//   [Resilience Decay]               where resilience may fail
//   [Resilience Reinforcement]       actions to maintain resilience
//   [Post-Stabilization Resistance]  per-dimension resistance words
...
// ----- Drivers + Inhibitors ----------------------------------------------
//
// Drivers match the Card 58 stabilization indicator vocabulary so the
// two views stay correlated. Inhibitors cover the residual risk
// categories that detract from durability.
...
function projectScore(current: ResilienceScore, drivers: number, inhibitors: number): ResilienceScore {
  // If more drivers than inhibitors, project one step up; if fewer,
  // step down; otherwise stay put. Capped at HIGH / floored at LOW.
...
function buildTrajectory(
  ...
  // Report the underlying direction even when the score is capped at
  // HIGH / floored at LOW so operators can see the push regardless.
...
// ----- Decay (where resilience may fail) ---------------------------------
...
// ----- Post-Stabilization Resistance block -------------------------------
...
export function buildStructuralResilience(
  ...
  // `diffs` + `stabilization` kept on the signature for spec parity;
  // Phase-1 resilience is computed from the structured signature
  // trends directly.
  void diffs;
  void stabilization;
  ...
  // Never-active short-circuit: resilience is HIGH by baseline. The
  // dimensions never carried load so they show "strong" resistance.
