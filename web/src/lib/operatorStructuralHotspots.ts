...
//
// Localization layer: ranks the runs and structural dimensions
// carrying the heaviest risk, summarises pair-by-pair hotspot
// evolution, projects which dimensions are still climbing, and
// closes with a rule-based summary sentence.
//
// Seven sections:
//
//   [Top Primitive Hotspots]       Phase-1 sentinel — primitive IDs
//                                  require lineage + overlay context.
//   [Top Run Hotspots]             top-3 runs by risk score.
//   [Structural-Dimension Hotspots] vol / drift / crit / upper / pressure
//                                  ranked by intensity + max value.
//   [Hotspot Clusters]             Phase-1 sentinel (primitive-level).
//   [Hotspot Evolution]            adjacent-pair notes from Card 51 diffs.
//   [Hotspot Trajectory]           direction of each dimension.
//   [System-Level Hotspot Summary] short synthesis paragraph.
...
// Same per-run risk score as Card 54 — keeps the two diagnostics in
// sync so hotspot rankings line up with the risk grid.
...
// ----- Structural-Dimension Hotspots --------------------------------------

interface DimRanking {
  label:      string;
  severity:   Severity;
  sortValue:  number;  // numeric for ties
  sequence:   string;  // "0 → 1 → 3 → 4"
}
...
export function buildStructuralHotspots(
  ...
  // `risk` kept on the signature for spec parity; Phase-1 hotspots
  // derive from the structured signature + diff inputs directly.
  void risk;
