...
//
// First predictive-reasoning instrument: projects the system's
// structural future by extrapolating per-run signatures + adjacent
// pair diffs. Eleven sections:
//
//   [Hydraulic Trajectory]                regime sequence + next
//   [Pressure Trajectory]                 pressure-band sequence + next
//   [Critical-Zone Trajectory]            count sequence + next
//   [Upper-Branch Trajectory]             count sequence + next
//   [Volatility Trajectory]               count sequence + next
//   [Drift Trajectory]                    count sequence + next
//   [Phase Trajectory]                    phase-label sequence + next
//   [Identity-Shift Trajectory]           identity-bucket sequence + next
//   [Projected Structural Risks]          rule-based bullet list
//   [Projected Structural Opportunities]  rule-based bullet list
//   [System-Level Trajectory Summary]     short synthesis paragraph
//
// All projections use simple, deterministic linear extrapolation
// (next = last + last_increment, bounded at 0). Qualitative notes
// flag continuation / plateau / saturation / lock-in patterns.
...
function detectOpportunities(
  ...
  // Hydraulic stabilization: last 2 regimes the same AND projected the same.
  if (
    sigs.length >= 2 &&
  ...
  // Pressure plateau: pressure band projected unchanged.
  if (sigs.length >= 2 && p.pressure.projected === last.pressureBand) {
    out.push("pressure plateau");
  }
  // Volatility containment: volatility decreased OR projected to decrease.
  if (sigs.length >= 2 && p.vol.projected < last.volatilityCount) {
    out.push("volatility containment");
  }
  // Drift normalization: drift = 0 OR projected to decrease.
  if (last.driftCount === 0 || p.drift.projected < last.driftCount) {
...
export function buildStructuralTrajectory(
  ...
  // `diffs` + `overlay` kept on the signature for parity with the
  // spec; Phase-1 projections derive from the structured signature
  // sequence directly. Diffs can be consulted by future risk rules.
  void diffs;
  void overlay;
