// Card 66 — Structural governance coherence engine (Phase-4, Tier-6).
//
// Combines Card 61 (governance), Card 62 (governance diff), Card 63
// (governance stability), Card 64 (governance resilience), and Card 65
// (governance immunity) text outputs into a 12-section coherence
// assessment:
//
//   [Coherence Level]             LOW … HIGH
//   [Governance Alignment]        weak / partial / strong
//   [Governance Consistency]      weak / moderate / strong
//   [Cross-Dimension Agreement]   weak / partial / strong
//   [Contradiction Risk]          low / moderate / elevated / high
//   [Coherence Trajectory]        gov → coherence → projected (state)
//   [Coherence Drivers]
...
// ----- Section parser (same pattern as Cards 62-65) -------------------
...
function parseVulnerability(imm: string): Vulnerability {
  const body = parseSection(imm, "Governance Vulnerability");
  if (body === "high")     return "high";
...
// Coherence floors on the weakest of the three upstream layers
// (stability / resilience / immunity) — if any one is low, the
// system can't be more coherent than that bottleneck. Penalties
// then apply on top for inhibitor density / threshold breach.
function coherenceLevelOf(
  ...
  let rank = Math.min(
    LEVEL_RANK[stability], LEVEL_RANK[resilience], LEVEL_RANK[immunity],
  );
...
function alignmentOf(stability: Level, resilience: Level, immunity: Level): Alignment {
...
type Consistency = "weak" | "moderate" | "strong";

function consistencyOf(integrity: Integrity, card63Coherence: Card63Coherence): Consistency {
  if (integrity === "strong" && card63Coherence === "strong") return "strong";
  if (integrity === "weak" || card63Coherence === "weak")     return "weak";
  return "moderate";
}

type Agreement = "weak" | "partial" | "strong";

function agreementOf(
  govLevel: Level, stability: Level, resilience: Level, immunity: Level,
): Agreement {
  const ranks = [
    LEVEL_RANK[govLevel], LEVEL_RANK[stability],
    LEVEL_RANK[resilience], LEVEL_RANK[immunity],
  ];
...
// Fires high when the layered governance levels are wildly out of
// sync, or vulnerability is high. Tiered downward by inhibitor
// density / vulnerability strength.
function contradictionRiskOf(
  inhibitorCount: number,
  vulnerability:  Vulnerability,
  layerSpread:    number,
): ContradictionRisk {
  if (vulnerability === "high" || layerSpread >= 2)             return "high";
  if (inhibitorCount >= 3 || vulnerability === "elevated")      return "elevated";
  if (inhibitorCount >= 1 || vulnerability === "moderate")      return "moderate";
  return "low";
}

// Trajectory walks gov → coherence → projected; tail mirrors Card 61's
// "(projected)" vs "(stable)" convention.
function buildCoherenceTrajectory(
  govLevel:  Level,
  coherence: Level,
  slope:     number,
): string {
  const cohRank  = LEVEL_RANK[coherence];
  const projRank = Math.max(0, Math.min(4, cohRank + slope));
  const proj     = RANK_TO_LEVEL[projRank];
  const moves    = govLevel !== coherence || coherence !== proj;
  const tail     = moves ? "(projected)" : "(stable)";
  return `${govLevel.toLowerCase()} → ${coherence.toLowerCase()} → ${proj.toLowerCase()} ${tail}`;
}
...
// Coherence layer reports the upstream improvement signals as one
// triple when slope is positive — operators reading the coherence
// pane see the whole stack moving together.
function collectDrivers(slope: number): string[] {
  const out: string[] = [];
  if (slope > 0) {
    uniqPush(out, "improving governance stability");
    uniqPush(out, "improving governance resilience");
    uniqPush(out, "improving immunity trajectory");
  }
  return out;
}

// Card 66 inhibitors pull directly from Card 61's [Governance
// Inhibitors] block (no "persistent" prefix) plus profile-derived
// weaknesses — the coherence pane is a snapshot of the live
// vulnerability surface, not a transition signal.
...
function collectRisks(
  ...
  const out: string[] = [];
  if (layerSpread >= 1)                       uniqPush(out, "cross-dimension divergence");
...
function collectReinforcement(profile: Profile, hardening: Hardening): string[] {
  const out: string[] = [];
...
// Card 66 decay order matches the spec demo: drift, volatility,
// thresholds (different from Card 64's threshold-first ordering).
...
// "partial" → "moderate" mapping for the summary, matching the spec
// demo where the [Cross-Dimension Agreement] block reads "partial"
// while the trailing sentence says "moderate".
function summaryAgreementWord(a: Agreement): string {
...
function buildSummary(
  ...
  if (direction === "improving") {
    s1 = inhibitors.length > 0
      ? `Governance coherence is improving but remains vulnerable to ${inhibitorPhrase} instability.`
      : `Governance coherence is improving with no remaining inhibitors.`;
  } else if (direction === "deteriorating") {
    s1 = inhibitors.length > 0
      ? `Governance coherence is deteriorating under ${inhibitorPhrase} pressure.`
      : `Governance coherence is deteriorating — reinforcement recommended.`;
  } else {
    s1 = inhibitors.length > 0
      ? `Governance coherence is steady but constrained by ${inhibitorPhrase}.`
      : `Governance coherence is steady with no material changes.`;
  }

  const s2 = `Alignment is ${alignment}, and cross-dimension agreement is ${summaryAgreementWord(agreement)}.`;
  return `${s1} ${s2}`;
}

// ----- Entry point -----------------------------------------------------

export function buildStructuralGovernanceCoherence(
  governance:           string,
  ...
  const vulnerability = parseVulnerability(governanceImmunity);
  const hardening     = parseHardening(governanceImmunity);

  // Derived fields.
  const inhibitors    = collectInhibitors(govInhibitors, profile);
  const coherence     = coherenceLevelOf(stability, resilience, immunity, inhibitors, profile);
  const alignment     = alignmentOf(stability, resilience, immunity);
  const consistency   = consistencyOf(integrity, card63Coherence);
  const agreement     = agreementOf(govLevel, stability, resilience, immunity);
  const layerSpread   = Math.max(
    LEVEL_RANK[govLevel], LEVEL_RANK[stability], LEVEL_RANK[resilience], LEVEL_RANK[immunity],
  ) - Math.min(
    LEVEL_RANK[govLevel], LEVEL_RANK[stability], LEVEL_RANK[resilience], LEVEL_RANK[immunity],
  );
  const contradiction = contradictionRiskOf(inhibitors.length, vulnerability, layerSpread);
  const trajectory    = buildCoherenceTrajectory(govLevel, coherence, slope);
  const drivers       = collectDrivers(slope);
  const risks         = collectRisks(layerSpread, profile, pressure, contradiction);
  const reinforcement = collectReinforcement(profile, hardening);
  const decay         = collectDecay(profile);
  const summary       = buildSummary(direction, alignment, agreement, inhibitors);

  const blocks: string[] = [];
  blocks.push("=== Structural Governance Coherence ===");
  blocks.push(`[Coherence Level]\n${coherence}`);
  blocks.push(`[Governance Alignment]\n${alignment}`);
  blocks.push(`[Governance Consistency]\n${consistency}`);
  blocks.push(`[Cross-Dimension Agreement]\n${agreement}`);
  blocks.push(`[Contradiction Risk]\n${contradiction}`);
  ...
  blocks.push(`[System-Level Coherence Summary]\n${summary}`);
