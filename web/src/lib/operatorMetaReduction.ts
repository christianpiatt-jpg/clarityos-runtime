...
//
// Third card of the final Operator Advanced Operator cluster (AO-9
// through AO-14, Cards 85-90) — the meta-consolidation cluster. If
// consolidation (85) gathers and compression (86) densifies, reduction
// (87) extracts the irreducible core of the meta-stack. Same weight
// class as Cards 77-86.
//
// Card 87 reduces all ten meta-operators into a single read — meta-
// pattern (77), meta-stability (78), meta-resilience (79), meta-
// immunity (80), meta-integration (81), meta-alignment (82), meta-
// coherence (83), meta-synthesis (84), meta-consolidation (85), and
// meta-compression (86). Each "X-Reduction" dimension reads the level
// of the corresponding meta-layer:
//
//   [Meta-Reduction Level]            LOW … HIGH
//   [Pattern-Reduction]               "<word> reduction"
//   [Stability-Reduction]             "<word> reduction"
//   [Resilience-Reduction]            "<word> reduction"
//   [Immunity-Reduction]              "<word> reduction"
//   [Integration-Reduction]           "<word> integration reduction"
//   [Alignment-Reduction]             "<word> alignment reduction"
//   [Coherence-Reduction]             "<word> coherence reduction"
//   [Synthesis-Reduction]             "<word> synthesis reduction"
//   [Consolidation-Reduction]         "<word> consolidation reduction"
//   [Compression-Reduction]           "<word> compression reduction"
//   [Meta-Reduction Trajectory]       curr → next → projected (state)
//   [Meta-Reduction Drivers]
//   [Meta-Reduction Inhibitors]
//   [Meta-Reduction Risks]
//   [Meta-Reduction Reinforcement]
//   [Meta-Reduction Decay]
//   [Operator Meta-Reduction Summary]
//
...
// Reduction word scale (weak < partial < moderate < strong).
type ReductionWord = "weak" | "partial" | "moderate" | "strong";

// ----- Section parser (same pattern as Cards 62-86) -------------------
...
// Maps a meta-layer level to a reduction word. HIGH (and above) and
// MEDIUM-HIGH read "strong", MEDIUM "moderate", LOW-MEDIUM "partial",
// LOW "weak".
function levelWord(level: Level): ReductionWord {
...
// Meta-reduction level is the floored average of the ten reduced facet
// levels, capped at HIGH (the level itself never reads VERY-HIGH — only
// the projected trajectory can reach that band).
function metaReductionLevelOf(levels: Level[]): Level {
...
// 78/80/82/85/86). Clamps to [0, 5] so an improving projection can
...
function collectDrivers(
  patternWord:    ReductionWord,
  resilienceWord: ReductionWord,
  synthesisWord:  ReductionWord,
): string[] {
  const out: string[] = [];
  if (patternWord === "strong")    out.push("strong pattern reduction");
  if (resilienceWord === "strong") out.push("strong resilience reduction");
  if (synthesisWord === "strong")  out.push("strong synthesis reduction");
  return out;
}

function collectInhibitors(
  immunityWord:  ReductionWord,
  stabilityWord: ReductionWord,
  pressure:      PressureLevel,
): string[] {
  const out: string[] = [];
  if (immunityWord !== "strong")  out.push(`${immunityWord} immunity reduction`);
  if (stabilityWord !== "strong") out.push(`${stabilityWord} stability reduction`);
...
function collectReinforcement(
  ...
  if (immunityWord === "strong") out.push("maintain immunity reduction");
  else                           out.push("strengthen immunity reduction");
...
function collectDecay(
  ...
  const out: string[] = [];
  if (pressure !== "low")   out.push("pressure may disrupt reduction");
...
function buildSummary(
  ...
  // synthesis-reduction all read the same word, otherwise name each.
  const allSame = patternWord === resilienceWord && resilienceWord === synthesisWord;
  const prs = allSame
    ? `${patternWord} pattern-, resilience-, and synthesis-reduction`
    : `${patternWord} pattern-, ${resilienceWord} resilience-, and ${synthesisWord} synthesis-reduction`;
  const s1 = `Operator meta-reduction is ${lead}, with ${prs}.`;
  if (immunityWord !== "strong") {
    return `${s1} Immunity-reduction remains ${immunityWord} and may disrupt overall reduction.`;
  }
  return s1;
}
...
export function buildOperatorMetaReduction(
  ...
  // callers. Card 87 reduces the ten meta-layers (77-86), which already
  // fold the operator core + system-operator integration stack.
  ...
  // Parse the ten reduced meta-layer levels.
  ...
  // Derived fields.
  const level = metaReductionLevelOf([
    ...
    metaConsolidationLevel, metaCompressionLevel,
  ]);
  ...
  const blocks: string[] = [];
  blocks.push("=== Operator Meta-Reduction ===");
  blocks.push(`[Meta-Reduction Level]\n${level}`);
  blocks.push(`[Pattern-Reduction]\n${patternWord} reduction`);
  blocks.push(`[Stability-Reduction]\n${stabilityWord} reduction`);
  blocks.push(`[Resilience-Reduction]\n${resilienceWord} reduction`);
  blocks.push(`[Immunity-Reduction]\n${immunityWord} reduction`);
  blocks.push(`[Integration-Reduction]\n${integrationWord} integration reduction`);
  blocks.push(`[Alignment-Reduction]\n${alignmentWord} alignment reduction`);
  blocks.push(`[Coherence-Reduction]\n${coherenceWord} coherence reduction`);
  blocks.push(`[Synthesis-Reduction]\n${synthesisWord} synthesis reduction`);
  blocks.push(`[Consolidation-Reduction]\n${consolidationWord} consolidation reduction`);
  blocks.push(`[Compression-Reduction]\n${compressionWord} compression reduction`);
  blocks.push(`[Meta-Reduction Trajectory]\n${trajectory}`);
  blocks.push(
    drivers.length === 0
      ? `[Meta-Reduction Drivers]\n(none)`
  ...
  blocks.push(`[Operator Meta-Reduction Summary]\n${summary}`);
