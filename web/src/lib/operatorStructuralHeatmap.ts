...
//
// Pure deterministic, client-side string builder that renders a
// compressed, symbol-only intensity map of the entire system. Each
// primitive × run cell collapses to two parts:
//
//   1) A single "pressure" symbol — `.` / `+` / `*` / `#` — derived
//      from a scalar pressure score (regime intensity + zone flags +
//      volatility + drift).
//   2) Trailing flag letters making the contributing factors visible:
//      `C` (in_critical_zone), `B` (on_upper_branch), `!` (regime
//      flipped at this transition), `~` (long-range drift).
//
// Absent primitives render as a blank cell. Column widths grow to
// fit the widest computed cell so output stays grid-aligned for any
// fixture.
...
// Regime intensity component of the pressure score.
//   laminar      → 0  (no pressure)
//   transitional → 1
//   turbulent    → 2
...
// Symbol bucket for the integer pressure score.
//   0    → "."
//   1    → "+"
//   2    → "*"
//   3+   → "#"
...
interface HeatmapDerived {
  // Per-primitive drift flag — applies on the primitive's last
  // active run only, same definition the Card 47 matrix uses.
...
export function buildStructuralHeatmap(
  ...
  // Overlay carried for signature parity; matrix reads lineage + evo.
  void overlay;
  ...
  // First pass: compute every cell as a raw token string so we can
  // size columns to the widest content.
  ...
  // Column width: max of widest cell, header label width, and 2 so
  // even all-stable systems get a readable grid.
