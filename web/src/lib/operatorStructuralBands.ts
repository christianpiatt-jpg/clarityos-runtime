...
// three-section banded structural signature:
//
//   [Run-Level Bands]               compressed per-run summary line
//                                   (band symbol + overlay flags)
//   [System-Level Phase Bands]      detected transitions between
//                                   adjacent runs (pressure, vol,
//                                   drift, crit-zone, upper-branch)
//   [Primitive-Level Band Summary]  rule-based primitive buckets
//
// The per-cell pressure formula matches Card 48 so the banding and
// heatmap stay correlated. Run-level pressure is the sum across all
// active primitives in that run, bucketed into 5 bands.
...
// Run-aggregate pressure bucket (matches the spec example).
...
// Compressed pressure-level name used in phase descriptions. Three
// levels (low / medium / high) gives operators clean transitions in
// the phase block while still respecting the five-band symbol scale.
...
export function buildStructuralBands(
  ...
  // Overlay + heatmap kept in the signature for future Phase-2 work
  // (Cards 50+ may consume them); banding reads only structured inputs.
  void overlay;
  void heatmap;
