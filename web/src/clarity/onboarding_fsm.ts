// web/src/clarity/onboarding_fsm.ts
//
// Canonical onboarding FSM (Onboarding Spec §1.1, panels 1–6, forward-only).
//
...
// in-memory only; cross-session resume should be wired to a vault_client
// (web/src/clarity/vault_client.ts, when materialized).
...
export function abort(): void {
  // Per Onboarding Spec §1.7 — return to Panel 1; payloads retained for
  // re-presentation. ACCEPTANCE timing markers are reset so the next
  // start is timed from scratch.
  _current = 1;
  _ts_ms_started = null;
...
// harness and dashboard. Caller serializes as needed for transport.
