...
export async function runElinsV2(req: ElinsV2RunRequest): Promise<ElinsV2Envelope> {
  ...
  // Defensive: the backend requires a non-empty raw_text and returns 400
  // otherwise. We short-circuit here to avoid an unnecessary round-trip.
  ...
  // The backend always returns the full envelope on 200; we cast after a
  // minimal shape check so consumers can rely on the types.
