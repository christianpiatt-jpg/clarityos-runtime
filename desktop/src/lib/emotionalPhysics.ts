// desktop/src/lib/emotionalPhysics.ts
//
// Typed client for POST /me/emotional_physics/analyze (v52).
//
// The backend (intelligence_kernel.run_emotional_physics) always
// returns the four top-level layer keys below; ``_meta.parse_error``
// is non-null when the model couldn't be parsed into a JSON object.
// Layer field shapes are kept permissive (Record<string, unknown>)
// because the per-layer fields are model-defined and evolve across
// passes — the view renderer treats them defensively.
//
// Layer names mirror the canonical set in lib/api.ts and on the
// backend exactly (see intelligence_kernel.py::_EMOTIONAL_PHYSICS_KEYS).
// Kept as its own module (not folded into lib/api.ts) because the
