...
//
// First card of the final Operator Advanced Operator cluster (AO-9
// through AO-14, Cards 85-90) — the meta-consolidation cluster. This
// cluster is post-integration and post-synthesis: it consolidates,
// compresses, and projects the meta-stack rather than building new
// structure. Same weight class as Cards 77-84.
//
// Card 85 consolidates all eight meta-operators into a single read —
// meta-pattern (77), meta-stability (78), meta-resilience (79), meta-
// immunity (80), meta-integration (81), meta-alignment (82), meta-
// coherence (83), and meta-synthesis (84). Each "X-Consolidation"
// dimension reads the level of the corresponding meta-layer:
//
//   [Meta-Consolidation Level]        LOW … HIGH
//   [Pattern-Consolidation]           "<word> consolidation"
//   [Stability-Consolidation]         "<word> consolidation"
//   [Resilience-Consolidation]        "<word> consolidation"
//   [Immunity-Consolidation]          "<word> consolidation"
//   [Integration-Consolidation]       "<word> integration consolidation"
//   [Alignment-Consolidation]         "<word> alignment consolidation"
//   [Coherence-Consolidation]         "<word> coherence consolidation"
//   [Synthesis-Consolidation]         "<word> synthesis consolidation"
//   [Meta-Consolidation Trajectory]   curr → next → projected (state)
//   [Meta-Consolidation Drivers]
//   [Meta-Consolidation Inhibitors]
//   [Meta-Consolidation Risks]
//   [Meta-Consolidation Reinforcement]
//   [Meta-Consolidation Decay]
//   [Operator Meta-Consolidation Summary]
//
// Like the Card 84 capstone, its projected trajectory can reach a
...
// Consolidation word scale (weak < partial < moderate < strong).
type ConsolidationWord = "weak" | "partial" | "moderate" | "strong";

// ----- Section parser (same pattern as Cards 62-84) -------------------
...
// Maps a meta-layer level to a consolidation word. HIGH (and above) and
// MEDIUM-HIGH read "strong", MEDIUM "moderate", LOW-MEDIUM "partial",
// LOW "weak".
function levelWord(level: Level): ConsolidationWord {
...
// Meta-consolidation level is the floored average of the eight
// consolidated facet levels, capped at HIGH (the level itself never
// reads VERY-HIGH — only the projected trajectory can reach that band).
function metaConsolidationLevelOf(levels: Level[]): Level {
...
// 78/80/82). Clamps to [0, 5] so an improving projection can reach the
// capstone VERY-HIGH band.
...
function collectDrivers(
  patternWord:    ConsolidationWord,
  resilienceWord: ConsolidationWord,
  synthesisWord:  ConsolidationWord,
): string[] {
  const out: string[] = [];
  if (patternWord === "strong")    out.push("strong pattern consolidation");
  if (resilienceWord === "strong") out.push("strong resilience consolidation");
  if (synthesisWord === "strong")  out.push("strong synthesis consolidation");
  return out;
}

function collectInhibitors(
  immunityWord:  ConsolidationWord,
  stabilityWord: ConsolidationWord,
  pressure:      PressureLevel,
): string[] {
  const out: string[] = [];
  if (immunityWord !== "strong")  out.push(`${immunityWord} immunity consolidation`);
  if (stabilityWord !== "strong") out.push(`${stabilityWord} stability consolidation`);
...
function collectReinforcement(
  ...
  if (immunityWord === "strong") out.push("maintain immunity consolidation");
  else                           out.push("strengthen immunity consolidation");
...
function collectDecay(
  ...
  const out: string[] = [];
  if (pressure !== "low")   out.push("pressure may disrupt consolidation");
...
function buildSummary(
  ...
  // synthesis-consolidation all read the same word, otherwise name each.
  const allSame = patternWord === resilienceWord && resilienceWord === synthesisWord;
  const prs = allSame
    ? `${patternWord} pattern-, resilience-, and synthesis-consolidation`
    : `${patternWord} pattern-, ${resilienceWord} resilience-, and ${synthesisWord} synthesis-consolidation`;
  const s1 = `Operator meta-consolidation is ${lead}, with ${prs}.`;
  if (immunityWord !== "strong") {
    return `${s1} Immunity-consolidation remains ${immunityWord} and may disrupt overall consolidation.`;
  }
  return s1;
}
...
export function buildOperatorMetaConsolidation(
  ...
  // callers. Card 85 consolidates the eight meta-layers (77-84), which
  ...
  // Parse the eight consolidated meta-layer levels.
  ...
  // Per-dimension consolidation words.
  ...
  // Derived fields.
  const level = metaConsolidationLevelOf([
  ...
  const blocks: string[] = [];
  blocks.push("=== Operator Meta-Consolidation ===");
  blocks.push(`[Meta-Consolidation Level]\n${level}`);
  blocks.push(`[Pattern-Consolidation]\n${patternWord} consolidation`);
  blocks.push(`[Stability-Consolidation]\n${stabilityWord} consolidation`);
  blocks.push(`[Resilience-Consolidation]\n${resilienceWord} consolidation`);
  blocks.push(`[Immunity-Consolidation]\n${immunityWord} consolidation`);
  blocks.push(`[Integration-Consolidation]\n${integrationWord} integration consolidation`);
  blocks.push(`[Alignment-Consolidation]\n${alignmentWord} alignment consolidation`);
  blocks.push(`[Coherence-Consolidation]\n${coherenceWord} coherence consolidation`);
  blocks.push(`[Synthesis-Consolidation]\n${synthesisWord} synthesis consolidation`);
  blocks.push(`[Meta-Consolidation Trajectory]\n${trajectory}`);
  blocks.push(
    drivers.length === 0
      ? `[Meta-Consolidation Drivers]\n(none)`
  ...
  blocks.push(`[Operator Meta-Consolidation Summary]\n${summary}`);
